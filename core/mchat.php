...
class mchat
{
	...
	protected function render_page($page)
	{
		...
		$this->template->assign_vars([
			...
			'MCHAT_MESSAGE_LNGTH'			=> $this->mchat_settings->cfg('mchat_max_message_lngth'),
			...
			'MCHAT_RULES'					=> $this->lang->lang('MCHAT_RULES_MESSAGE') ?: $this->mchat_settings->cfg('mchat_rules'),
			'MCHAT_SESSION_TIMELEFT'		=> $this->lang->lang('MCHAT_SESSION_ENDS', gmdate($this->mchat_settings->cfg('mchat_timeout') >= 3600 ? 'H:i:s' : 'i:s', $this->mchat_settings->cfg('mchat_timeout'))),
			...
			'MCHAT_REFRESH_RATE'			=> $this->mchat_settings->cfg('mchat_refresh'),
	...
	public function assign_messages($rows, $page = '')
	{
		...
		foreach ($rows as $row)
		{
			...
			$template_data = [
				...
				'MCHAT_U_PERMISSIONS'		=> append_sid($this->mchat_settings->url('adm/index', true), ['i' => 'permissions', 'mode' => 'setting_user_global', 'user_id[0]' => $row['user_id']], true, $this->user->session_id),
